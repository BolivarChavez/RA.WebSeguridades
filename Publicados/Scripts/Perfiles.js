async function CallServerMethod() {
    ...
    $.ajax({
        ...
        cache: false,
        type: "POST",
        url: "Perfiles.aspx/ConsultaPerfiles",
...
async function LlenaGrid() {
    ...
    var grid = new ej.grids.Grid({
        ...
        height: '100%',
        columns: [
            { field: 'pe_codigo', headerText: 'Codigo', visible: false, width: 20, textAlign: 'Left', customAttributes: { class: 'boldheadergrid' } },
            { field: 'pe_descripcion', headerText: 'Descripcion', width: 150, textAlign: 'Left', customAttributes: { class: 'boldheadergrid' } },
            { field: 'pe_estado', headerText: 'Estado', width: 150, textAlign: 'Left', customAttributes: { class: 'boldheadergrid' } }
        ],
...
function rowSelected(args) {
    document.getElementById('Codigo').value = args.data.pe_codigo;
    document.getElementById('Descripcion').value = args.data.pe_descripcion;
    document.getElementById("chkEstado").checked = (args.data.pe_estado === 'A') ? true : false;
}
...
function ValidaDatos() {
    ...
    if (descripcion.trim() === "") {
        document.getElementById('messageContent').innerHTML = "ERROR : No se ha ingresado una descripción para el perfil";
...
function GrabarPerfil() {
    ...
    if (confirm("Confirma la grabación del registro del perfil?")) {
        strParametro = "";
        ...
        $.ajax({
            ...
            cache: false,
            type: "POST",
            url: "Perfiles.aspx/GrabarPerfil",
            data: "{" + args + "}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
...
function EliminarPerfil() {
    ...
    if (confirm("Confirma la eliminación del registro del perfil?")) {
        strParametro = "";
        strParametro += document.getElementById('Codigo').value + "|";
        strParametro += document.getElementById('Descripcion').value + "|";
        strParametro += "X";
